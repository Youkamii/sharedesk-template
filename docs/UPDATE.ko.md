...
관리자 화면 → 설정 → 업데이트에서 `자동 업데이트`를 켜면, 켠 컴퓨터의 시간대 기준 자정에 새 버전이 자동으로 적용됩니다. 별도의 키가 필요 없습니다. 켜져 있는 동안 작업표시줄의 업데이트 버튼은 숨겨지고, 업데이트 내용은 같은 설정 화면에서 볼 수 있습니다. 켜려면 GitHub에서 ShareDesk 저장소에 별을 남겨야 합니다.

이 기능은 저장소의 `.github/workflows/sharedesk-auto-update.yml` 워크플로가 처리하며, 새로 설치한 저장소에는 처음부터 들어 있습니다. 예전에 설치한 저장소에 이 파일이 없다면, 원본 저장소의 같은 경로에서 파일 내용을 복사해 내 저장소의 같은 위치에 한 번만 추가해 주세요.

알아 둘 점: 자동 업데이트는 사람 확인 없이 새 릴리스를 그대로 적용합니다. 원본 저장소를 신뢰할 수 있을 때만 켜 주세요 — 원본 계정이 탈취되면 그 코드가 자동으로 들어옵니다.

참고: GitHub는 저장소에 60일 동안 활동이 없으면 예약 실행을 잠시 멈춥니다. 그때는 저장소의 Actions 화면에서 `ShareDesk Auto Update` 워크플로를 다시 활성화하면 됩니다.
