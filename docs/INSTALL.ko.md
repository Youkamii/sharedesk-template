...
**설치 주소 하나가 데스크 하나입니다.** 한 설치 안에서 계정이 여러 데스크를 만들거나 전환하는 구조는 없습니다. 같은 Google 계정이 서로 다른 ShareDesk 주소에 각각 초대될 수는 있지만, 사용자 명단·역할·저장 용량 제한·채팅·공유 링크는 주소마다 따로 관리됩니다.
...
이 역할은 현재 데스크 전체에 적용됩니다. 한 설치 안에는 별도 프로젝트나 폴더별 ShareDesk 권한이 없으므로 승인된 사용자는 같은 Drive 루트의 항목을 함께 보고, 역할에 따라 만들기·수정하기 범위만 달라집니다. 아래의 Google Drive 직접 공유 권한은 이 데스크 역할과 별개입니다.
...
관리자는 `/admin`의 **저장 용량**에서 한 파일의 최대 업로드 크기와 현재 데스크 전체 한도를 GB 단위로 정할 수 있습니다. 예를 들어 호스트 Drive 한도가 5 TB여도 이 데스크에는 500 GB만 허용할 수 있습니다. 값이 비어 있으면 ShareDesk 제한을 두지 않으며, 실제 저장은 언제나 호스트 Drive의 남은 용량도 함께 따라야 합니다. 여러 파일을 한꺼번에 골라도 한 파일마다 업로드 제한을 검사하고, 동시에 올라가는 파일은 예약 용량까지 합쳐 데스크 한도를 넘지 못하게 합니다.
...
- 호스트 Google Drive의 실제 용량과 휴지통 보관 기간은 Google 정책을 따릅니다. ShareDesk 관리자는 그 안에서 현재 데스크에 더 작은 전체 한도와 한 파일 업로드 한도를 둘 수 있습니다.

Drive 모드에서는 `ShareDesk/.sharedesk/`에 사용자·초대, 현재 접속 인원, 채팅, 외부 공유 링크, 업로드 예약, Drive 공유 권한, 폴더 메모와 아이콘 배치를 저장합니다. 일반 파일 목록에서는 이 폴더를 숨깁니다. 동시에 같은 상태를 바꾸면 먼저 저장한 결과를 유지하고 늦은 요청은 충돌로 끝내 최신 상태를 다시 읽게 합니다.
