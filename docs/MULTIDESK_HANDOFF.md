...
- 입장할 때 닉네임 입력(첫 입장, 닉이 없을 때), 자기 이름 클릭으로 변경
  (`PATCH /api/me/nickname` 백엔드는 이미 있음)
- 접속 인원 표시·우측 하단 이름을 닉네임으로 교체
- 관리자 화면: 구글 이름과 닉네임 병기 + 닉 변경 기록(`nicknameHistory`는
  이미 `GET /api/admin/users` 응답에 내려온다 — 표시만 붙이면 된다)
- 주의(인수인계 "나중에" 항목과 연결): `setUserNickname`은 스페이스
  문맥에서 돌면 스페이스 명단에 쓴다. 정체는 기본 데스크가 진실
  원천이므로 라우트에서 기본 문맥으로 고정할지 이 단계에서 결정하라
