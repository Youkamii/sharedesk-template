...
## 最後の手順: 会話記録の整理を提案してください

インストールの会話には、callback URLや画面貼り付けに混ざった秘密の値、環境変数の内容が残りがちです。インストール完了を告げた直後、AIは必ず次のように提案してください。

> この会話には秘密の値が残っている可能性があります。チャット記録を削除しましょうか？

ユーザーが同意したら、ツールごとの記録ファイルを削除します。進行中のセッションファイルは会話が終わるまで書き込まれ続けるため、「この会話を閉じたあとに下のコマンドを実行するか、次のセッションで私に削除を指示してください」と案内しても構いません。

- Claude Code: `%USERPROFILE%\.claude\projects\<プロジェクトフォルダ>\<セッションid>.jsonl`（macOS/Linuxは `~/.claude/projects/...`）
- Codex CLI: `~/.codex/sessions` 配下の該当セッションファイル

注意: 削除で消えるのはこのコンピュータ上の記録だけです。すでにサービス提供者のサーバーへ送信された内容は各社の保存ポリシーに従います。そのため、露出した秘密の値は削除とは別に再発行するのが最も確実です。
