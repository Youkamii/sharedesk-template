[English](./LOCAL.md) · **한국어** · [日本語](./LOCAL.ja.md) · [हिन्दी](./LOCAL.hi.md) · [中文](./LOCAL.zh.md)

# ShareDesk 로컬 개인 사용

Google OAuth나 Vercel 없이 내 컴퓨터에서 ShareDesk 화면과 파일 기능을 쓰는 방법입니다. 파일은 Google Drive가 아니라 이 컴퓨터의 로컬 폴더에 저장됩니다.

이 방식은 개인 사용과 개발 확인에 맞습니다. 여러 사람이 각자의 Google 계정으로 함께 쓰는 운영 환경을 만들려면 [운영 설치 안내](./INSTALL.ko.md)를 따르세요. 설치가 어렵다면 [AI에게 구축 맡기기](./AI_INSTALL.ko.md)를 사용할 수 있습니다.

이미 받은 로컬 설치본을 새 버전으로 바꾸는 방법은 [업데이트 안내](./UPDATE.ko.md#로컬-개인-사용)에 따로 정리했습니다.
...
Vercel 기본 도메인을 쓰면서 `PUBLIC_BASE_URL`을 비우면 앱은 Vercel이 제공하는 `VERCEL_PROJECT_PRODUCTION_URL`을 사용합니다. 직접 넣는 값이 아니라 Vercel의 시스템 환경 변수입니다. 운영에 필요한 값과 callback 주소는 [운영 설치 안내](./INSTALL.ko.md)에 정리돼 있습니다.
