...
관리자로 ShareDesk에 로그인한 뒤 작업표시줄에서 `업데이트`를 누릅니다.

1. 현재 버전과 최신 안정 버전을 확인합니다.
2. 새 버전이 있으면 `업데이트 시작`을 누릅니다.
3. 열린 GitHub Actions 화면에서 `Run workflow`를 누릅니다.
4. 검사가 모두 통과하면 설치 저장소의 `main`에 업데이트 커밋이 생깁니다.
5. 저장소와 연결된 Vercel이 새 커밋을 Production에 배포합니다.

검사나 빌드가 실패하면 `main`에 커밋하지 않으므로 현재 운영 배포는 그대로 남습니다. 설치본에서 ShareDesk 코드 파일을 직접 고친 경우에도 덮어쓰지 않고 충돌한 파일 이름을 표시하며 멈춥니다.

GitHub 기본 권한으로 실행 중인 workflow 파일 자체를 바꾸는 것은 허용되지 않습니다. 나중에 ShareDesk의 업데이트 작업 방식이 바뀐 릴리스에서는 구버전 workflow를 조용히 유지하지 않고 작업을 멈추며, 아래의 `0.2.0보다 오래된 설치를 한 번 전환하기`와 같은 1회 전환을 안내합니다. 앱과 일반 업데이트 코드는 그 경우가 아니면 화면 버튼으로 계속 갱신됩니다.
