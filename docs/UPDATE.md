...
GitHub 기본 권한으로 실행 중인 workflow 파일 자체를 바꾸는 것은 허용되지 않습니다. 나중에 ShareDesk의 업데이트 작업 방식이 바뀐 릴리스에서는 구버전 workflow를 조용히 유지하지 않고 작업을 멈추며, 아래의 `0.2.0보다 오래된 설치를 한 번 전환하기`와 같은 1회 전환을 안내합니다. 앱과 일반 업데이트 코드는 그 경우가 아니면 관리자가 위 절차를 직접 시작했을 때만 갱신됩니다.
