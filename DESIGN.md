...
- **What this is:** 한 Google Drive 폴더를 승인된 사람들이 함께 쓰는 웹 기반 공유 바탕화면이다.
...
- **Display/UI:** Galmuri11 — 한글이 흐트러지지 않는 실제 비트맵 계열 글꼴. 12px 배수에서 가장 선명하게 쓴다.
- **Body:** Galmuri11, 그다음 Geist Sans. 긴 설명은 14px 이상과 넉넉한 줄 간격을 사용한다.
- **Data:** Geist Mono — 용량, 시간, 진행률처럼 숫자 정렬이 필요한 곳에만 사용한다.
- **Loading:** `/public/fonts/Galmuri11.woff2`를 자체 호스팅한다. OFL 1.1 라이선스 파일을 함께 둔다.
- **Scale:** 11, 12, 14, 16, 20, 28px. 픽셀 글꼴은 임의의 중간 크기를 만들지 않는다.

## Color

- **Approach:** 표현력 있는 배경 위에 제한된 UI 팔레트.
- **Night:** `#10172b` — 가장 깊은 배경과 그림자.
- **Panel:** `#182446` — 창과 작업표시줄의 기본 면.
- **Title:** `#2f4670` — 활성 창 제목줄.
...
- **Icon footprint:** 약 88×94px. 위치는 화면 크기와 무관한 논리 픽셀 좌표로 저장하고, 좁은 화면에서는 바탕화면 안쪽을 스크롤한다.
...
- 폴더는 더블클릭/Enter로 창에서 열고, 파일은 더블클릭/Enter로 내려받는다.
...
- 빈 폴더와 오류는 작은 창 문구로 설명하고 사용 가능한 다음 행동을 함께 둔다.
