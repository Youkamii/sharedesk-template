...
| 다른 사람의 ShareDesk 참여하기 | 먼저 Google 계정으로 로그인한 뒤 관리자가 발급한 기간제 초대 코드를 입력합니다. OAuth나 Vercel 설정은 필요 없습니다. |
...
- 마지막에는 운영 URL에서 호스트 Google 로그인, /files 폴더 생성, 새로고침 뒤 유지, /admin 접근, 기간제 초대 코드 발급 화면을 실제로 확인해.
...
```text
이 저장소의 README와 현재 ShareDesk 화면을 보고 관리자 사용법을 안내해줘. 사용자 한 명에게 기간제 초대 코드 발급하기, 폴더와 파일 올리기, Google Drive 보기 권한으로 공유하기, 휴지통에서 복원하기, 특정 기기 로그인 끊기를 순서대로 진행하자. 내가 한 단계를 마쳤다고 말하면 다음 단계로 넘어가고, 삭제나 권한 회수처럼 되돌리기 어려운 작업은 실행 전에 확인해줘.
```
...
- 초대 코드는 호스트가 정한 기간에 지정한 Google 계정에서 한 번만 사용됩니다. 관리 화면에는 사용자 차단, 특정 기기 로그인 끊기, 모든 로그인 끊기 기능이 있습니다.
...
각 데스크의 관리자 ── 기간제 1회용 초대 코드 ──▶ 그 데스크의 참여자
```
...
5. 운영 확인 뒤 `/admin`에서 참여자별 기간제 초대 코드를 만드세요.
...
3. 초대 코드의 유효 기간을 `1시간`, `24시간`, `7일`, `30일` 중에서 고릅니다.
4. 필요하면 비고를 남기고 초대 코드를 만듭니다.
5. 표시된 코드를 받을 사람에게 안전한 채널로 전달합니다.
6. 받는 사람은 ShareDesk에서 먼저 지정된 Google 계정으로 로그인합니다.
7. 로그인 뒤 나타나는 가입 화면에 코드를 입력하면 바로 승인됩니다.

초대 코드는 일회용입니다. 만료됐거나 비활성화됐거나 이미 사용한 코드는 거부됩니다. 지정한 이메일과 다른 Google 계정에서도 작동하지 않습니다. 새 코드를 발급하면 예전 코드는 즉시 무효가 됩니다. 같은 유효 기간이 발급 시점부터 다시 시작됩니다.
...
| 가입 대기로 변경 | 접근을 멈춥니다. 사용자는 다시 로그인한 뒤 새 초대 코드를 입력해야 합니다. |
...
| 초대 코드가 거부됨 | 로그인한 Google 이메일, 코드의 만료일·활성 상태·사용 여부를 `/admin`에서 확인하세요. 새 코드를 발급했다면 예전 코드는 더 이상 쓸 수 없습니다. |
