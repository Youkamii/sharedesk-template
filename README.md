...
- 키가 유출되면 키를 아는 모두가 접근 가능하다. **회수하려면 `ACCESS_KEYS`에서 그 키를 빼고 재배포/재시작**하면 된다 — 세션 토큰에 어떤 키로 들어왔는지가 새겨져 있어서, 키를 빼는 즉시 그 키로 만든 기존 세션도 전부 무효가 된다.
- 키 추측 시도는 IP당 분당 10회로 제한된다(인스턴스 단위).
