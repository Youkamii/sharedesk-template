...
호스트 한 사람의 **Google Drive 저장 공간을 여러 사람이 함께 쓰는 공유 파일 공간**입니다. 모두 같은 ShareDesk 주소에 각자의 Google 계정으로 들어와 파일을 올리고, 폴더를 정리하고, 같은 파일을 볼 수 있습니다.

설치는 호스트가 처음에 한 번만 합니다. **초대받은 참여자는 GitHub, Vercel, OAuth를 설정할 필요가 없습니다.** 호스트가 보낸 주소에서 Google로 로그인하고 초대 코드만 넣으면 됩니다.

![오른쪽 아래에 고정된 휴지통과 열린 폴더 창이 보이는 ShareDesk 바탕화면](./docs/sharedesk-desktop.png)

화면 오른쪽 아래의 휴지통은 작업표시줄 버튼이 아니라 바탕화면에 따로 고정된 아이콘입니다. 열린 창보다 뒤에 놓여 작업 중인 창을 가리지 않습니다.

| 하고 싶은 일 | 시작 방법 |
|---|---|
| 이미 만들어진 ShareDesk에 참여하기 | 호스트가 보낸 주소에서 내 Google 계정으로 로그인하고 초대 코드를 입력합니다. GitHub, Vercel, OAuth 설정은 필요 없습니다. |
| 내 Drive 용량을 함께 쓸 ShareDesk 열기 | 호스트만 [설치 안내](./docs/INSTALL.md)에 따라 내 저장소, Vercel 프로젝트, Google Drive를 한 번 연결합니다. 이후에는 주소와 초대 코드만 공유하면 됩니다. |

빠른 링크: [설치 안내](./docs/INSTALL.md) · [OAuth 없이 실행](#oauth-없이-로컬에서-실행하기) · [사람 초대하기](#사람-초대하기) · [문제 해결](#문제-해결)

## 새 ShareDesk의 호스트가 되기
...
이 절차는 자기 Google Drive 용량을 내어 새 공유 공간을 열 **호스트**만 진행합니다. 초대받아 같은 공간을 쓸 참여자는 설치하지 않습니다.
...
- 휴지통 아이콘은 작업표시줄이 아닌 화면 오른쪽 아래에 고정되어 있습니다. 아이콘을 누르면 삭제한 항목을 복원하거나 완전히 삭제할 수 있습니다. Drive 모드는 Google Drive 정책에 따라, `local` 모드는 ShareDesk가 같은 기준으로 30일 동안 보관합니다.
...
- 데스크의 분위기를 바꾸는 배경을 고를 수 있으며, 선택은 개인 브라우저에만 저장됩니다.

## 어떻게 동작하나요?

```text
호스트의 Google Drive · ShareDesk 폴더
                       ↕
             같은 ShareDesk 주소
             ├── 호스트의 Google 계정
             ├── 참여자 A의 Google 계정
...
- Google Drive에 연결하는 계정은 호스트 한 명입니다. 호스트와 승인된 참여자들은 같은 폴더와 파일을 보고, 호스트의 Drive 용량을 함께 씁니다.
- 참여자는 각자의 Google 계정으로 로그인합니다. 호스트가 만든 초대 코드를 처음 한 번 입력하면 같은 공간에 들어오며, 참여자의 개인 Drive 파일은 읽지 않습니다.
...
- 아이콘 배치, 사용자·초대·공유 상태, 폴더 이동처럼 버전을 확인하는 작업은 뒤늦은 쓰기를 거부하고 최신 상태를 다시 불러옵니다.

새 ShareDesk를 여는 **호스트**는 자신의 GitHub 저장소, Vercel 프로젝트, Google OAuth 클라이언트, Drive 루트를 한 번 연결합니다. 이 구조 덕분에 호스트가 열은 서로 다른 ShareDesk 끼리는 파일·사용자·초대 정보를 공유하지 않습니다. 참여자에게는 이 설치가 필요 없습니다.
...
- Drive 모드에서 휴지통으로 보낸 항목은 직접 완전히 삭제하지 않는 한 [Google Drive 정책](https://support.google.com/drive/answer/14933051?hl=ko)에 따라 30일이 지나면 영구 삭제됩니다. OAuth 없는 `local` 모드에서는 30일이 지난 항목을 다음 휴지통 조회 때 ShareDesk가 영구 삭제합니다.
